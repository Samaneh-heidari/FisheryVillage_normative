...
public class DecisionMaker {
	...
	public double calculatePreferenceAccordingToValues(double donationAmount) {
		...
		double imprtThres = new Double(important.split(";")[1]);
		
		if(imprtValName.equals(AbstractValue.TRADITION.name()) || imprtValName.equals(AbstractValue.UNIVERSALISM.name()))
				return donationAmount * imprtThres / 100.0;
			if(imprtValName.equals(AbstractValue.SELFDIRECTION.name()) || imprtValName.equals(AbstractValue.POWER.name()))
				return donationAmount * (100 - imprtThres) / 100.0;
