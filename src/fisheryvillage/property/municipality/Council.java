...
public class Council extends Workplace {
	...
	public int getNumberOfAge(int minimum, int maximum, String groupName) {
		ArrayList<Human> humans = SimUtils.getObjectsAll(Human.class);
		int count = 0;
		for (Human human : humans) {
			if (human.getAge() >= minimum && human.getAge() < maximum && human.isMember(groupName)) {
				count ++;
			}
		}
		return count;
	...
	public int getNumberOfAdultsCheapNeighborhood() {
		return getNumberOfAge(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.CHEAP.name());
	}

	public int getNumberOfAdultsStandardNeighborhood() {
		return getNumberOfAge(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.STANDARD.name());
	}
	
	public int getNumberOfAdultsEXPENSIVENeighborhood() {
		return getNumberOfAge(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.EXPENSIVE.name());
	}

	public int getNumberOfAgeAndValue(int minimum, int maximum, String groupName, String valueName) {
		ArrayList<Resident> residents = SimUtils.getObjectsAll(Resident.class);
		int count = 0;
		...
		for (Resident resident : residents) {
			...
			if (imprtValName.equals(valueName) && resident.getAge() >= minimum && resident.getAge() < maximum && resident.isMember(groupName)) {
				count ++;
			}
		}
	...
	public int getNumberOfAdultsCheapUniversalism() {
		return getNumberOfAgeAndValue(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.CHEAP.name(), AbstractValue.UNIVERSALISM.name());
	}

	public int getNumberOfAdultsStandardUniversalism() {
		return getNumberOfAgeAndValue(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.STANDARD.name(), AbstractValue.UNIVERSALISM.name());
	}
	
	public int getNumberOfAdultsEXPENSIVEUniversalism() {
		return getNumberOfAgeAndValue(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.EXPENSIVE.name(), AbstractValue.UNIVERSALISM.name());
	}

	public int getNumberOfAdultsCheapPower() {
		return getNumberOfAgeAndValue(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.CHEAP.name(), AbstractValue.POWER.name());
	}

	public int getNumberOfAdultsStandardPower() {
		return getNumberOfAgeAndValue(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.STANDARD.name(), AbstractValue.POWER.name());
	}
	
	public int getNumberOfAdultsEXPENSIVEPower() {
		return getNumberOfAgeAndValue(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.EXPENSIVE.name(), AbstractValue.POWER.name());
	}

	public int getNumberOfAdultsCheapTradition() {
		return getNumberOfAgeAndValue(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.CHEAP.name(), AbstractValue.TRADITION.name());
	}

	public int getNumberOfAdultsStandardTradition() {
		return getNumberOfAgeAndValue(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.STANDARD.name(), AbstractValue.TRADITION.name());
	}
	
	public int getNumberOfAdultsEXPENSIVETradition() {
		return getNumberOfAgeAndValue(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.EXPENSIVE.name(), AbstractValue.TRADITION.name());
	}

	public int getNumberOfAdultsCheapSelfdirection() {
		return getNumberOfAgeAndValue(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.CHEAP.name(), AbstractValue.SELFDIRECTION.name());
	}

	public int getNumberOfAdultsStandardSelfdirection() {
		return getNumberOfAgeAndValue(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.STANDARD.name(), AbstractValue.SELFDIRECTION.name());
	}
	
	public int getNumberOfAdultsEXPENSIVESelfdirection() {
		return getNumberOfAgeAndValue(Constants.HUMAN_ADULT_AGE, Constants.HUMAN_ELDERLY_AGE, HouseType.EXPENSIVE.name(), AbstractValue.SELFDIRECTION.name());
	}
